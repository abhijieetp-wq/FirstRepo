...
/**
 * Kept as the name the spare picker has always called. The work is shared with the compressor
 * catalogue now — see CatalogComplete.gs for why that matters.
 */
function completeSpareDetails(input) {
  return completeCatalogDetails({
    itemType: 'Spare', itemId: input.spareId,
    hsnCode: input.hsnCode, price: input.price, quoteNo: input.quoteNo
  });
}
